...
try {
    ...
    Write-Host "OK — synced=$($response.synced), unmatched=$($response.unmatched), rows=$($response.csv_rows), took=$($response.duration_ms)ms"
    if ($response.unmatched -gt 0) {
        Write-Warning "$($response.unmatched) items could not be matched. See $logFile for details."
    }
    exit 0
}
catch {
