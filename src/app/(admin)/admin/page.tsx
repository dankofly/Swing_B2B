    ...
    <div className="space-y-8">
      ...
      <div className="dash-hero rounded-xl px-8 py-9">
        <div className="relative z-10 flex items-end justify-between">
          ...
          <p className="text-right text-xs font-medium text-white/25">
      ...
      <div className="overflow-hidden card ">
        ...
        {!recentInquiries || recentInquiries.length === 0 ? (
          ...
          <>
            {/* Table header */}
            <div className="grid grid-cols-[1fr_7rem_5rem_9rem] items-center gap-3 border-t border-gray-100 bg-gray-50/60 px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.15em] text-swing-navy/40">
              <span>Händler</span>
              ...
              {recentInquiries.map((inquiry: any) => {
                ...
                return (
                  <Link
                    key={inquiry.id}
                    href="/admin/anfragen"
                    className="grid cursor-pointer grid-cols-[1fr_7rem_5rem_9rem] items-center gap-3 px-6 py-3.5 transition-colors duration-150 hover:bg-swing-gold/4"
                  >
                    ...
                    <div className="flex justify-center">
                      <span
                        className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold ${status.bg} ${status.color}`}
                      >
                        {status.label}
                      </span>
                    </div>
                    <span className="truncate text-center text-xs text-swing-gray-dark/35">
                      {(inquiry as any).user?.full_name?.split(" ")[0] || "—"}
                    </span>
                    <span className="text-right text-xs tabular-nums text-swing-gray-dark/35">
